...
def test_basic_functionality():
    ...
    assert p.print_details() == "Product: Laptop, Price: $1000", (
        f"Print details failed: {p.print_details()}"
    )
    assert p.apply_discount(10) == 900, (
        f"Discount calculation failed: {p.apply_discount(10)}"
    )

    physical = PhysicalProduct("Desk", 500)
    physical.set_weight(30)
    assert physical.calculate_shipping() == 15, (
        f"Shipping calculation failed: {physical.calculate_shipping()}"
    )

    digital = DigitalProduct("Software", 200)
    assert digital.apply_discount(10) == 180, (
        f"Digital discount failed: {digital.apply_discount(10)}"
    )
...
def test_edge_cases():
    ...
    assert p.apply_discount(10) == 0, (
        f"Zero price discount failed: {p.apply_discount(10)}"
    )

    p_neg = Product("Negative Item", -100)
    assert p_neg.apply_discount(10) == -90, (
        f"Negative price discount failed: {p_neg.apply_discount(10)}"
    )

    physical = PhysicalProduct("Empty Box", 10)
    physical.set_weight(0)
    assert physical.calculate_shipping() == 0, (
        f"Zero weight shipping failed: {physical.calculate_shipping()}"
    )

    physical_neg = PhysicalProduct("Anti-Gravity Item", 10)
    physical_neg.set_weight(-5)
    assert physical_neg.calculate_shipping() == -2.5, (
        f"Negative weight shipping failed: {physical_neg.calculate_shipping()}"
    )
...
def test_large_values():
    ...
    assert p.apply_discount(10) == 900000, (
        f"Large value discount failed: {p.apply_discount(10)}"
    )

    physical = PhysicalProduct("Heavy Item", 500)
    physical.set_weight(1000)
    assert physical.calculate_shipping() == 500, (
        f"Large weight shipping failed: {physical.calculate_shipping()}"
    )
...
def test_inheritance():
    ...
    assert isinstance(physical, ShippableMixin), (
        "PhysicalProduct should inherit from ShippableMixin"
    )
    assert isinstance(physical, PrintableMixin), (
        "PhysicalProduct should inherit from PrintableMixin through Product"
    )
    assert isinstance(physical, DiscountMixin), (
        "PhysicalProduct should inherit from DiscountMixin through Product"
    )

    assert isinstance(digital, Product), "DigitalProduct should inherit from Product"
    assert isinstance(digital, PrintableMixin), (
        "DigitalProduct should inherit from PrintableMixin through Product"
    )
    assert isinstance(digital, DiscountMixin), (
        "DigitalProduct should inherit from DiscountMixin through Product"
    )
...
def test_method_overriding():
    ...
    assert p.apply_discount(20) == 80, (
        f"Regular discount calculation failed: {p.apply_discount(20)}"
    )
    assert digital.apply_discount(20) == 90, (
        f"Digital fixed discount failed: {digital.apply_discount(20)}"
    )

    # Digital product should always apply 10% discount regardless of parameter
    assert digital.apply_discount(0) == 90, (
        "Digital product should apply 10% discount even with 0%"
    )
    assert digital.apply_discount(50) == 90, (
        "Digital product should apply 10% discount even with 50%"
    )
...
def test_polymorphism():
    ...
    for i, product in enumerate(products):
        ...
        assert "Product:" in product.print_details(), (
            f"Polymorphic print_details failed for {type(product)}"
        )

        # All should have apply_discount method but with different implementations
        assert product.apply_discount(20) == expected_discounts[i], (
            f"Polymorphic apply_discount failed for {type(product)}"
        )
...
def test_attribute_access():
    ...
    assert physical.name == "Physical Product", (
        "Name attribute not properly set in PhysicalProduct"
    )
    ...
    assert digital.name == "Digital Product", (
        "Name attribute not properly set in DigitalProduct"
    )
