...
class Bank:
    ...
    def create_account(
        self, account_type, account_number, owner_name, initial_balance=0, **kwargs
    ):
    ...
    def transfer(self, from_account_number, to_account_number, amount):
        ...
        success, message = to_account.deposit(amount)
        if not success:
            return False, f"Transfer failed: {message}"
        return (
            True,
            f"Transferred ${amount:.2f} from {from_account_number} to {to_account_number}",
        )
