...
if test_case == "character_creation":
    ...
    print(
        f"Warrior: {warrior.name}, HP: {warrior.health}, Mana: {warrior.mana}, Str: {warrior.strength}"
    )
    print(
        f"Mage: {mage.name}, HP: {mage.health}, Mana: {mage.mana}, Int: {mage.intelligence}"
    )
    print(
        f"Rogue: {rogue.name}, HP: {rogue.health}, Mana: {rogue.mana}, Agi: {rogue.agility}"
    )
    ...
    print(
        f"Warrior - Level: {warrior.level}, HP: {warrior.max_health}, Str: {warrior.strength}"
    )
    print(
        f"Mage - Level: {mage.level}, Mana: {mage.max_mana}, Int: {mage.intelligence}"
    )
    ...
    print(
        f"Initial HP - {warrior.name}: {warrior.health}, {enemy.name}: {enemy.health}"
    )
    ...
    print(
        f"{warrior.name}: {warrior.health}/{warrior.max_health} HP, {warrior.mana}/{warrior.max_mana} Mana"
    )
    print(
        f"{mage.name}: {mage.health}/{mage.max_health} HP, {mage.mana}/{mage.max_mana} Mana"
    )
    ...
    while warrior.is_alive() and mage.is_alive():
        ...
        print(
            f"Status: {warrior.name} ({warrior.health:.1f} HP, {warrior.mana} Mana) | {mage.name} ({mage.health:.1f} HP, {mage.mana} Mana)"
        )
        round_num += 1
