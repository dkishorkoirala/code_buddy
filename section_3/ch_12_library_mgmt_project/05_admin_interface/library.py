...
class Library:
    ...
    def get_statistics(self):
        ...
        available_book = len([book for book in self.books if book.available])
        borrowed_books = total_books - available_book
        total_user = len(self.users)
        user_with_books = len([user for user in self.users if user.books_borrowed])

        return {
            "total_books": total_books,
            "available_books": available_book,
            "borrowed_books": borrowed_books,
            "total_users": total_user,
            "users_with_books": user_with_books,
        }
